...
class TestCase(unittest.TestCase):
    ...
    def test_config_knowledge_organization(self):
        try:
            ...
            time.sleep(2)
            menuVertical = self.driver.find_element(By.XPATH, '//div[@class = "workbench-menu-vertical"]')
            menuVertical.find_elements(By.CLASS_NAME, 'el-menu-item')[1].click()
            time.sleep(2)
            knowledge = self.driver.find_elements(By.XPATH, '//div[@class="workbench-content"]')[2]
            formContainerList = knowledge.find_elements(By.CLASS_NAME, 'form-container')
    ...
    def test_config_knowledge_work(self):
        try:
            canvas = self.driver.find_elements(By.CLASS_NAME, 'files-page')[0]
            canvas.find_element(By.TAG_NAME, 'pre')
            actions = ActionChains(self.driver).move_to_element(canvas).click_and_hold().move_by_offset(100,
            ...
            for item in addFormSection.find_elements(By.CSS_SELECTOR, '.el-form-item.is-required'):
                ...
                if isTextarea:
                    ...
                    currentValue = item.find_element(By.CLASS_NAME, 'el-input__inner').get_attribute('value')
                self.assertEqual(labelValue[label], currentValue,
                                 '测试知识数据-->作品-->' + label.replace(':', '') + '表单编辑功能成功!')
            ...
            try:
                ...
                message = ''
            self.assertEqual('删除成功！', message, '删除知识数据-->作品记录成功')
        except Exception as e:
            logging.exception(e)


    def test_config_knowledge_inkind(self):
        try:
            canvas = self.driver.find_elements(By.CLASS_NAME, 'files-page')[0]
            canvas.find_element(By.TAG_NAME, 'pre')
            actions = ActionChains(self.driver).move_to_element(canvas).click_and_hold().move_by_offset(100,
            ...
            time.sleep(2)
            menuVertical = self.driver.find_element(By.XPATH, '//div[@class = "workbench-menu-vertical"]')
            menuVertical.find_elements(By.CLASS_NAME, 'el-menu-item')[5].click()
            time.sleep(2)
            knowledge = self.driver.find_elements(By.XPATH, '//div[@class="workbench-content"]')[2]
            formContainerList = knowledge.find_elements(By.CLASS_NAME, 'form-container')
            formSectionList = formContainerList[5].find_elements(By.CLASS_NAME, 'form-section')
            addFormSection = formSectionList[len(formSectionList) - 1]
            labelValue = {}
            for item in addFormSection.find_elements(By.CSS_SELECTOR, '.el-form-item.is-required'):
                label = item.find_element(By.CLASS_NAME, 'el-form-item__label').text
            ...
            for item in addFormSection.find_elements(By.CSS_SELECTOR, '.el-form-item.is-required'):
                ...
                if isTextarea:
                    ...
                    currentValue = item.find_element(By.CLASS_NAME, 'el-input__inner').get_attribute('value')
                self.assertEqual(labelValue[label], currentValue,
                                 '测试知识数据-->实物-->' + label.replace(':', '') + '表单编辑功能成功!')
            ...
            try:
                ...
                message = ''
            self.assertEqual('删除成功！', message, '删除知识数据-->实物记录成功')
        except Exception as e:
            logging.exception(e)

    def test_config_knowledge_gragment(self):
        try:
            self.driver.find_element(By.XPATH, '//span[text() = "创建内容片段知识"]').click()
            time.sleep(1)

            files = self.driver.find_element(By.ID, 'files')
            firstFilePage = files.find_elements(By.CLASS_NAME, 'files-page')[0]
            firstFilePage.find_element(By.CLASS_NAME, 'el-checkbox__inner').click()
            time.sleep(1)
            ...
            knowledge = self.driver.find_elements(By.XPATH, '//div[@class="workbench-content"]')[2]
            formContainerList = knowledge.find_elements(By.CLASS_NAME, 'form-container')
            formSectionList = formContainerList[6].find_elements(By.CLASS_NAME, 'form-section')
            addFormSection = formSectionList[len(formSectionList) - 1]
            addFormSection.find_element(By.CLASS_NAME, 'el-icon-edit').click()
            time.sleep(1)
            for item in addFormSection.find_elements(By.CSS_SELECTOR, '.el-form-item.is-required'):
            ...
            for item in addFormSection.find_elements(By.CSS_SELECTOR, '.el-form-item.is-required'):
                ...
                if isTextarea:
                    ...
                    currentValue = item.find_element(By.CLASS_NAME, 'el-input__inner').get_attribute('value')
                self.assertEqual(labelValue[label], currentValue,
                                 '测试知识数据-->知识内容片段-->' + label.replace(':', '') + '表单编辑功能成功!')
            ...
            try:
                ...
                message = ''
            self.assertEqual('删除成功！', message, '删除知识数据-->知识内容片段记录成功')
        except Exception as e:
            logging.exception(e)
    ...
    def test_graininess_count(self):
        try:
            ...
            geographyCount = self.driver.find_element(By.ID, 'tab-geography').text.replace('地理名称 (', '').replace(')',
                                                                                                                 '')
            workCount = self.driver.find_element(By.ID, 'tab-work').text.replace('作品 (', '').replace(')',
                                                                                                     '')
            inkindCount = self.driver.find_element(By.ID, 'tab-inkind').text.replace('实物 (', '').replace(')',
                                                                                                         '')
            fragmentCount = self.driver.find_element(By.ID, 'tab-fragment').text.replace('知识内容片段 (', '').replace(')',
                                                                                                                 '')
            formContainerList = knowledge.find_elements(By.CLASS_NAME, 'form-container')
            ...
            self.assertEqual(len(formContainerList[1].find_elements(By.CLASS_NAME, 'form-section')), int(eventCount),
                             '知识数据事件计数校验成功')
            self.assertEqual(len(formContainerList[2].find_elements(By.CLASS_NAME, 'form-section')),
            ...
            self.assertEqual(len(formContainerList[4].find_elements(By.CLASS_NAME, 'form-section')), int(workCount),
                             '知识数据作品计数校验成功')
            self.assertEqual(len(formContainerList[5].find_elements(By.CLASS_NAME, 'form-section')), int(inkindCount),
                             '知识数据实物计数校验成功')
            self.assertEqual(len(formContainerList[6].find_elements(By.CLASS_NAME, 'form-section')), int(fragmentCount),
                             '知识数据知识内容片段计数校验成功')

            self.assertEqual(
                int(personCount) + int(eventCount) + int(organizationCount) + int(geographyCount) + int(
                    workCount) + int(inkindCount) + int(fragmentCount),
