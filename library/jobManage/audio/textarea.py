...
class TestCase(unittest.TestCase):
    ...
    def test_edit_text(self):
        try:
            self.driver.find_elements(By.XPATH, '//span[text() = "编辑"]')[0].click()
            time.sleep(1)

            files = self.driver.find_element(By.ID, 'files')
            firstFilePage = files.find_elements(By.CLASS_NAME, 'files-page')[0]
            elTextArea = firstFilePage.find_element(By.CLASS_NAME, 'el-textarea__inner')
            ...
            files = self.driver.find_element(By.ID, 'files')
            firstFilePage = files.find_elements(By.CLASS_NAME, 'files-page')[0]
            firstFilePagePre = firstFilePage.find_element(By.TAG_NAME, 'pre')

            self.assertEqual(firstFilePagePre.text.replace(' ', ''), '测试文本编辑功能',
                             '测试编辑文本区功能成功!')
