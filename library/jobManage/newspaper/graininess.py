...
class TestCase(unittest.TestCase):
    ...
    def test_graininess_count(self):
        try:
            ...
            text = graininess.find_elements(By.CLASS_NAME, 'form-container')[1]
            table = graininess.find_elements(By.CLASS_NAME, 'form-container')[0]
            ...
            textCount = self.driver.find_element(By.ID, 'tab-paper').text.replace('报纸 (', '').replace(')', '')
            ...
            self.assertEqual(len(textItems), int(textCount), '颗粒度报纸计数校验成功')
    ...
    def test_add_img_graininess_diagramm(self):
        try:
            self.driver.find_element(By.ID, 'tab-resource').click()
            time.sleep(1)
            ...
            self.driver.find_elements(By.XPATH,
                                      '//div[contains(@class, "item-panel") and  contains(@class, "text") and @data-title = "img"]')[
                0].click()
            time.sleep(2)
            try:
    ...
    def test_edit_img_graininess(self):
        try:
            self.driver.find_element(By.ID, 'tab-resource').click()
            time.sleep(1)
            graininess = self.driver.find_elements(By.XPATH, '//div[@class="workbench-content"]')[1]
    ...
    def test_graininess_remove_img(self):
        try:
            self.driver.find_element(By.ID, 'tab-resource').click()
            time.sleep(1)
            graininess = self.driver.find_elements(By.XPATH, '//div[@class="workbench-content"]')[1]
    ...
    def test_add_table_graininess_diagramm(self):
        try:
            self.driver.find_element(By.ID, 'tab-resource').click()
            time.sleep(1)
            self.driver.find_element(By.ID, 'tab-table').click()
            ...
            time.sleep(1)
            self.driver.find_elements(By.XPATH,
                                      '//div[contains(@class, "item-panel") and  contains(@class, "text") and @data-title = "table"]')[
                0].click()
            time.sleep(2)
            try:
                message = self.driver.find_element(By.CLASS_NAME, 'el-message__content').text
