...
class TestCase(unittest.TestCase):
    ...
    def test_account_manage(self):
        try:
            ...
            requiredFormItemList = baseInfo.find_elements(By.XPATH, '//div[contains(@class, "el-form-item") and  contains(@class, "is-required")]')

            label = requiredFormItemList[0].find_element(By.CLASS_NAME, 'el-form-item__label').text
            input = requiredFormItemList[0].find_element(By.CLASS_NAME, 'el-input__inner')
            input.clear()
            ...
            #     input = formItem.find_element(By.CLASS_NAME, 'el-input__inner')
            #     input.clear()
            #     input.send_keys('测试'+label.replace(':', ''))
            #     time.sleep(1)
            ...
            label = requiredFormItemList[0].find_element(By.CLASS_NAME, 'el-form-item__label').text
            inputValue = requiredFormItemList[0].find_element(By.CLASS_NAME, 'el-input__inner').get_attribute('value')
            self.assertIn('测试' + label.replace(':', ''), inputValue, '测试' + label.replace(':', '') + '表单编辑功能成功!')

            # for formItem in requiredFormItemList:
            #     label = formItem.find_element(By.CLASS_NAME, 'el-form-item__label').text
            #     input = formItem.find_element(By.CLASS_NAME, 'el-input__inner')
            #     self.assertIn('测试'+label.replace(':', ''), input.text, '测试'+label.replace(':', '')+'表单编辑功能成功!')
