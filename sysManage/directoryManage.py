    ...
    def test_org_manage(self):
        try:
            ...
            for row in el_rows:
                ...
                self.assertEqual(cols[4].text, firstTreeNodeText, '点击树节点查询结果符合要求')

            el_form_contents = self.driver.find_elements(By.XPATH, '//div[@class="el-form-item__content"]')
            orgNameInput = el_form_contents[0].find_element(By.CLASS_NAME, 'el-input__inner')
            orgCodeInput = el_form_contents[1].find_element(By.CLASS_NAME, 'el-input__inner')

            orgNameInput.clear()
            orgNameInput.send_keys('ceshi')
            ...
            self.assertEqual(orgNameInput.get_attribute('value'), '', '重置功能正确')
            self.assertEqual(orgCodeInput.get_attribute('value'), '', '重置功能正确')

            self.driver.find_element(By.XPATH, '//span[text()="新建"]').click()
            time.sleep(1)
            dialog = self.driver.find_element(By.XPATH, '//div[@class="el-dialog" and @aria-label="新增机构"]')
            ...
            formInputs[5].send_keys('机构名称-测试')
            formInputs[6].send_keys('机构全称-测试')
            formInputs[7].send_keys('机构简称-测试')
            ...
            orgNameInput.clear()
            orgCodeInput.clear()
            orgNameInput.send_keys('机构名称-测试')
            orgCodeInput.send_keys(timestamp)
            self.driver.find_element(By.XPATH, '//span[text()="查询"]').click()
            time.sleep(2)
            el_rows = self.driver.find_elements(By.XPATH, '//tr[@class="el-table__row"]')
