    ...
    def test_role_manage(self):
        try:
            ...
            for row in el_rows:
                ...
                self.assertIn('test', cols[0].text.lower(), '根据角色标识查询正确')
            ...
            for row in el_rows:
                cols = row.find_elements(By.TAG_NAME, 'span')
                self.assertIn('test', cols[1].text.lower(), '根据角色名称查询正确')
            ...
            checkBoxArray = dialogBody.find_elements(By.XPATH, '//span[@class="el-checkbox__input"]')
            formInputs = dialogBody.find_elements(By.XPATH, '//input[@class="el-input__inner"]')
            timestamp = int(time.time())
            formInputs[4].send_keys('角色标识-测试' + str(timestamp))
            formInputs[5].send_keys('角色名称-测试' + str(timestamp))
            checkBoxArray[0].click()
            self.driver.find_element(By.CSS_SELECTOR,
                                     '.el-dialog__footer:nth-child(3) .el-button--primary > span').click()
            time.sleep(2)
            roleId.clear()
            roleName.clear()
            roleId.send_keys('角色标识-测试' + str(timestamp))
            roleName.send_keys('角色名称-测试' + str(timestamp))
            self.driver.find_element(By.XPATH, '//span[text()="查询"]').click()
            time.sleep(2)
            el_rows = self.driver.find_elements(By.XPATH, '//tr[@class="el-table__row"]')
            self.assertEqual(1, len(el_rows), '根据新增参数查询记录为一条')
            cols = el_rows[0].find_elements(By.TAG_NAME, 'span')
            self.assertEqual('角色标识-测试' + str(timestamp), cols[0].text, '新增和查询记录的角色标识相同')
            self.assertEqual('角色名称-测试' + str(timestamp), cols[1].text, '新增和查询记录的角色名称相同')
            self.assertEqual('', cols[2].text, '新增和查询记录的互斥角色相同')
            self.assertIn('系统管理', cols[3].text, '新增和查询记录资源名称相同')
            self.assertEqual('是', cols[4].text, '新增和查询记录的是否编辑相同')

            self.driver.find_element(By.XPATH, '//span[text()="编辑"]').click()
            time.sleep(1)
            dialog = self.driver.find_element(By.XPATH, '//div[@class="el-dialog" and @aria-label="编辑角色"]')
            dialogBody = dialog.find_element(By.XPATH, '//div[@class="el-dialog__body"]')
            formInputs = dialogBody.find_elements(By.XPATH, '//input[@class="el-input__inner"]')
            formInputs[8].clear()
            formInputs[8].send_keys('角色名称-测试编辑' + str(timestamp))
            saveButton = self.driver.find_elements(By.CSS_SELECTOR,
                                                   '.el-dialog__footer:nth-child(3) .el-button--primary > span')
            saveButton[1].click()
            time.sleep(2)
            roleName.clear()
            roleName.send_keys('角色名称-测试编辑' + str(timestamp))
            self.driver.find_element(By.XPATH, '//span[text()="查询"]').click()
            time.sleep(2)
            el_rows = self.driver.find_elements(By.XPATH, '//tr[@class="el-table__row"]')
            cols = el_rows[0].find_elements(By.TAG_NAME, 'span')
            self.assertEqual('角色名称-测试编辑' + str(timestamp), cols[1].text, '编辑功能正常')
...
if __name__ == '__main__':
    ...
    br.report(filename='roleManage.html', description='测试报告',
