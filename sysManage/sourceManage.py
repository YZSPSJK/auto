...
class TestCase(unittest.TestCase):
    ...
    def test_org_manage(self):
        try:
            ...
            sourceNameInput.clear()
            sourceTypeInput.clear()
            sourceTypeInput.send_keys('菜单')
            self.driver.find_element(By.XPATH, '//span[text()="查询"]').click()
            time.sleep(2)
            el_rows = self.driver.find_elements(By.XPATH, '//tr[@class="el-table__row"]')
            for row in el_rows:
            ...
            sourceNameInput.clear()
            sourceTypeInput.clear()
            sourceNameInput.send_keys('资源名称')
            sourceTypeInput.send_keys('菜单')
            ...
            formInputs[4].send_keys('资源名称-测试')
            formInputs[5].send_keys('菜单')
            formInputs[7].send_keys(timestamp)
            formInputs[8].send_keys('匿名访问')
            ...
            sourceNameInput.clear()
            sourceTypeInput.clear()
            sourceNameInput.send_keys('资源名称-测试')
            sourceTypeInput.send_keys('菜单')
            self.driver.find_element(By.XPATH, '//span[text()="查询"]').click()
            time.sleep(2)
            el_rows = self.driver.find_elements(By.XPATH, '//tr[@class="el-table__row"]')
            ...
            self.assertEqual('资源名称-测试', cols[1].text, '新增和查询记录的资源名称相同')
            self.assertEqual('菜单', cols[2].text, '新增和查询记录的资源类型相同')
            self.assertEqual(firstTreeNodeText, cols[3].text, '新增和查询记录的上级资源相同')
            self.assertEqual('/system/*', cols[4].text, '新增和查询记录的ANT匹配格式相同')
            self.assertEqual('/system', cols[5].text, '新增和查询记录的访问路径相同')
            self.assertEqual('匿名访问', cols[6].text, '新增和查询记录的访问控制级别相同')
            self.assertEqual(str(timestamp), cols[7].text, '新增和查询记录的优先级相同')
            ...
            formInputs = dialogBody.find_elements(By.XPATH, '//input[@class="el-input__inner"]')
            formInputs[12].clear()
            formInputs[12].send_keys('资源名称-测试编辑')
            ...
            el_rows = self.driver.find_elements(By.XPATH, '//tr[@class="el-table__row"]')
            cols = el_rows[0].find_elements(By.TAG_NAME, 'span')
            self.assertEqual('资源名称-测试编辑', cols[1].text, '编辑功能正常')
